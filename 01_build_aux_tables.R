...
likelihood_tbl <- likelihood_of_being_fuel_supplier_by_nace %>%
  transmute(nace4d = as.character(nace4d), likelihood = as.character(likelihood)) %>%
  distinct()
...
customs_siec <- ef_weighted_fuel_qty %>%
  ...
  left_join(
    siec_to_ipcc %>% transmute(ipcc_fuel = as.character(ipcc_fuel), siec_code = as.character(siec_code)),
    by = "ipcc_fuel"
  )
...
firm_year_fuel_totals <- customs_siec %>%
  ...
  mutate(intensity_kg_per_eur = if_else(imports_value_total > 0, co2_kg_total / imports_value_total, NA_real_))

saveRDS(firm_year_fuel_totals, file.path(CACHE_DIR, "aux_firm_year_fuel_totals.rds"))
...
buyer_sector_siec <- siec_used_for_energy_by_sector %>%
  transmute(nace2d = as.character(nace2d), siec_code = as.character(siec_code)) %>%
  distinct()
saveRDS(buyer_sector_siec, file.path(CACHE_DIR, "aux_buyer_sector_siec.rds"))
