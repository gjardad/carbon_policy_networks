...
# Histogram plot of sufficient statistic as pct of total emissions
  ...
  for(y in 2005:2022){
    ...
    df <- df_cov_list[[i]] %>% 
      ...
      mutate(pct_total_emissions = cov/total_emissions*100,
    ...
    max_value <- ceiling(max(df$pct_total_emissions, na.rm = TRUE))
    min_value <- floor(min(df$pct_total_emissions, na.rm = TRUE))
    
    if (max_value < 10) {
      breaks <- seq(0, 8, by = 2)  # Breaks for max_value less than 10
    } else {
  ...
  # save it
  save(hist_suff_stat_pct_total_emissions_list, file = paste0(proc_data,"/histograms_for_suff_statistic_as_pct_total_emissions.RData"))
...
# Bar plot of total value of sufficient statistic as pct of total, EUETS sectors, and EUETS firms emissions
  ...
  for(y in 2005:2022){
    ...
    df <- df_cov_list[[i]] %>% 
      filter(vat == "total") %>% 
      mutate(pct_total_emissions = cov/total_emissions*100,
             pct_euets_sectors = cov/euets_sectors_emissions*100,
             pct_euets_emissions = cov/euets_emissions*100) %>% 
    ...
    y_upper_limit <- max(df_long$Value) + 5  # Set an upper limit with a buffer
    ...
    plot <- ggplot(df_long, aes(x = Emissions, y = Value, fill = Emissions, alpha = alpha)) +
      ...
      geom_text(aes(label = round(Value)), vjust = -0.5, size = 5) +  # Add rounded text labels on top of bars
  ...
  # save it
  save(hist_total_suff_stat_list, file = paste0(proc_data,"/histograms_total_suff_statistic.RData"))
