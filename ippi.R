...
## This code cleans the ippi Belgian data from Eurostat

# It produces a data set at the level of the nace code-year with the ippi index
...
# Create ippi by sector-year -----

  # Initialize an empty data frame to store results
  ippi_data <- data.frame()

  # Find data sets within raw_data/Eurostat folder
  file_folder <- paste0(raw_data, "/Eurostat")
  file_names <- list.files(path = file_folder, pattern = "^eurostat_ippi", full.names = FALSE)
  ...
  for (file in file_names){
    ...
    for (sheet in sheet_names) {
      ...
      ippi_data <- rbind(ippi_data, temp_df)
    }
  }
  
  #save(ippi_data, file = paste0(int_data,"/ippi_data.RData"))

  # Reshape the data to wide format with columns for each index year
  library(tidyr)
  ippi_wide <- pivot_wider(ippi_data, 
  ...
  ippi_wide <- ippi_wide %>% 
    ...
    mutate(
      ...
      ippi_2010 = coalesce(index_2010, index_2015, index_2021)
    ) %>%
    ungroup() %>% 
    select(c(code, year, ippi_2010))
  
  #save(ippi_wide, file = paste0(int_data,"/ippi_wide.RData"))
  
# Create most disaggregated ippi by sector-year
  ...
  # step 4: create ippi_final
    
    # step 4.1: create ippi_final in ippi_wide
    ippi_wide$ippi_final <- ippi_wide$ippi_2010
    
    # step 4.2: loop over rows with missing ippi_2010
    missing_rows <- which(is.na(ippi_wide$ippi_2010))
    
    for (i in missing_rows) {
      code <- ippi_wide$code[i]
      year <- ippi_wide$year[i]
      ...
      for (j in 1:7) {
        ...
        # Check if the parent_code has a non-missing ippi_2010 value in the same year
        ippi_value <- ippi_wide$ippi_2010[ippi_wide$code == parent_code & ippi_wide$year == year]
        
        if (length(ippi_value) > 0 && !is.na(ippi_value)) {
          # Assign the first non-missing ippi_2010 from the parent code to ippi_final
          ippi_wide$ippi_final[i] <- ippi_value
          break
        }
      }
    }
  
    # step 4.3: clean and save ippi_wide
    ippi_final <- ippi_wide %>% 
      select(-c(ippi_2010))
  
    save(ippi_final, file = paste0(proc_data,"/ippi_final.RData"))
